var globeApp = {
    ...
    calcIconScaleSize: function() {
        ...
        return this.iconSize*zoomScale;
    },
...
//            globeApp.onTextureLoaded(globeApp, 'texture', 'textureImg', 'textureCanBeUsed', e);
        }
    ...
    setIconPTextureImg: function(i, img) {
        var icon = this.icons[i];
//console.log('setIconPTextureImg:', i, icon?icon.n:'???', img.loaded, img.src);
        if (!icon) return false; 
        ...
        for (var i = 0;i<iconsCount;i++) {
            ...
            if (isNaN(lat) || isNaN(lng) || (lat<-90) || (lat>90) || (lng<-180) || (lng>180)) {
                ...
                this.icons.push({
                    ...
                    rT: r0,
                    r0: r0,
                    textureImgSrc: icon.src,
                    pTextureImgSrc: icon.pSrc,
                    textureCanBeUsed: false,
                    pTextureCanBeUsed: false,
                    rotX: 0,
                    rotY: 0,
        ...
        for (var i = 0;i<iconsDataCount;i++) {
            ...
            for (var j = 0;j<this.iconsCount;j++) {
                ...
                if (icon.code.toUpperCase()==iconData.code.toUpperCase()) {
                    icon.x2D = iconData.destX+15;
                    icon.y2D = iconData.destY+15;
                    break;
                }
            }
        }
    ...
    iconsUpdate: function(t2, t1) {
        ...
        for (var i = 0;i<this.iconsCount;i++) {
            ...
            if (pickIcon && (pickIcon.n==icon.n)) {
                ...
                icon.scale = glUtils.easeOutQuad(idt0, factor0, (this.overScale-factor0), this.overScaleDuration);
        ...
        if (this.moving2StartPos) {
            if (iconsOnTarget==this.iconsCount) {
...
//        this.gl.pixelStorei(this.gl.UNPACK_FLIP_Y_WEBGL, true); //for texture
        ...
        for (var i = 0;i<this.iconsCount;i++) {
            this.repaintObj('icon'+i, this.icons[i], offbuffer);
        }
        for (var objName in this.objects) {
