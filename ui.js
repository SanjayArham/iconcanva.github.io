(function($) {
    $(document).ready(function(){
        ...
        uiPaginationInit('awardsitems-next', 'uiFetchItems');
        uiPaginationInit('awardsitems-prev', 'uiFetchItems');
        ...
        var hash = uiGetHash();
        var infoPgs = ['news', 'about', 'lectures', 'shop', 'media', 'big-ideas', 'jobs', 'staff', 'contact', 'search', 'awards'];
        ...
        if (pgNameId>-1) {
            ...
            $("#contentblock").trigger("scroll"); //make sense for all infoPgs except jobs, staff, search
        }
        if (hash=='') location.hash = '#projects';
        
        $('.emailme').click(function(){
            location.href = 'mailto:'+$(this).attr('data-fp')+'@'+$(this).attr('data-lp');
        });
        $('.emailme').each(function() {
            $(this).html($(this).attr('data-fp') + '&#64;' + $(this).attr('data-lp'));
        });
    });
})(jQuery);
...
function uiOnHashChange(hash) {
    ...
    var menuItems = ['projects', 'news', 'about', 'lectures', 'shop', 'media', 'big-ideas', 'awards', 'jobs', 'staff', 'contact', 'search'];
...
uiMainPreloading.preloads1 = {//first preloads
    ...
    'big-ideas': {loaded: 0, toLoad: 2},
    'awards': {loaded: 0, toLoad: 5},
